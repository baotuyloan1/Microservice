...
To install a Package to the Kubernetes cluster:
```cmd
...
When using this command, it will have some content inside the templates folder. Make sure we clean the content inside the templates folder to create new own contents.

To compile the helm chart accounts, and it will also try to compile all the dependent helm charts and place them inside the chart folder.
```cmd
...
To see all the Kubernetes files that are going to be generated by your helm.
```cmd
helm template .
```
